...
- 🚧 **v1.2 Polish & Local-First UX** — Phases 6-9 (in progress)
...
- [x] **Phase 9: v1.2 Audit Gap Closure** - Clear `cargo clippy -- -D warnings` debt (33 errors across 14 files in `src-tauri/src/`), author the missing `07-VERIFICATION.md`, refresh `08-UAT.md` frontmatter — addresses gaps surfaced by `/gsd:audit-milestone` (see `.planning/milestones/v1.2-MILESTONE-AUDIT.md`) (completed 2026-05-28)
...
### Phase 9: v1.2 Audit Gap Closure

**Goal**: Close the v1.2 audit chain so the milestone can archive with a clean evidence trail — Phase 7 sampling-rate gate is green again, the missing Phase 7 verification artifact exists, and the stale Phase 8 UAT frontmatter reflects the closure work in plans 08-08/09/10.
**Depends on**: Phases 6, 7, 8 complete (this is the audit-cleanup phase)
**Requirements**: AUDIT-01 (clippy gate), AUDIT-02 (07-VERIFICATION backfill — promotes SHUT-01/02/03 from partial → satisfied), AUDIT-03 (08-UAT frontmatter refresh)
**Success Criteria** (what must be TRUE):

1. `cargo clippy --manifest-path src-tauri/Cargo.toml --all-targets -- -D warnings` exits 0 from the repo root
2. `.planning/phases/07-macos-clean-shutdown/07-VERIFICATION.md` exists with `status: passed` frontmatter and a populated truths / artifacts / link tables sourced from `07-01-SUMMARY.md`
3. `.planning/phases/08-privacy-local-first-ux/08-UAT.md` frontmatter reads `status: passed` and references the closure commits (Plans 08-08, 08-09, 08-10)
4. Re-running `/gsd:audit-milestone v1.2` reports `status: passed` (no partial requirements, no integration gaps)
   **Plans**: 1 plan (4 surgical tasks — branch merge prerequisite + clippy sweep + 07-VERIFICATION.md backfill + 08-UAT.md frontmatter refresh; ~60 min)

- [ ] 09-01-PLAN.md — Merge feat/phase-08-local-first-ux + clippy sweep (33 fixes across 14 files) + retroactive 07-VERIFICATION.md + 08-UAT.md frontmatter refresh (AUDIT-01, AUDIT-02, AUDIT-03)
