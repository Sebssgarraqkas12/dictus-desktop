# Requirements Archive: v1.2 Polish & Local-First UX
...
### Audit Gap Closure (AUDIT)

<!-- Added 2026-05-28 after /gsd:audit-milestone v1.2 surfaced tech_debt. See .planning/milestones/v1.2-MILESTONE-AUDIT.md. -->

- [x] **AUDIT-01**: `cargo clippy --manifest-path src-tauri/Cargo.toml --all-targets -- -D warnings` exits 0 from repo root (restores the sampling-rate gate claimed by `07-VALIDATION.md`; current state: 31 errors in `src-tauri/src/lib.rs:612/651/681` and surrounding unchanged code paths)
- [x] **AUDIT-02**: `.planning/phases/07-macos-clean-shutdown/07-VERIFICATION.md` exists with `status: passed` frontmatter and populated truths / artifacts / link tables — promotes SHUT-01/02/03 from `partial` to `satisfied` in the next audit pass
- [x] **AUDIT-03**: `.planning/phases/08-privacy-local-first-ux/08-UAT.md` frontmatter reads `status: passed` and body references the closure commits from plans 08-08, 08-09, 08-10 (current state: still `status: diagnosed` from 2026-05-22 partial-pass)
...
- v1.2 active requirements: 18 total (15 original + 3 AUDIT added 2026-05-28; after 2026-05-21 rescope: 11 cancelled — SYNC-07..11 + AGENT-01..06)
- Mapped to phases: 18 ✓
- Unmapped: 0 ✓
...
_Last updated: 2026-05-28 — AUDIT-01/02/03 added for Phase 9 gap closure (clippy gate, 07-VERIFICATION backfill, 08-UAT refresh) following `/gsd:audit-milestone v1.2` tech_debt result_
